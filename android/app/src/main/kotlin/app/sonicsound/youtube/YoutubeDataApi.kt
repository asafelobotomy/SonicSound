...
/** Official YouTube Data API v3 search with music-video matching. */
object YoutubeDataApi {
    fun search(apiKey: String, query: String, maxResults: Int = 20): List<YoutubeVideo> {
        if (apiKey.isBlank() || query.isBlank()) return emptyList()
        val q = URLEncoder.encode(query, Charsets.UTF_8.name())
        val url =
            "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video" +
                "&maxResults=$maxResults&q=$q&key=${URLEncoder.encode(apiKey, Charsets.UTF_8.name())}"
    ...
    /**
     * Find a music video for [artist]/[title].
     * Default prefers official / VEVO / artist channels and "Official Music Video" titles.
     * When [allowAnyChannel] is true, any channel may win if the title matches the song.
     */
    fun searchMusicVideo(
        ...
        for (q in queries) {
            for (v in search(apiKey, q, maxResults = 8)) {
    ...
    private fun score(
        ...
        val officialTitle = listOf(
            "official music video", "official video", "official mv", "official music video"
        ).any { vt.contains(it) }
        ...
        val artistChannel = a.isNotBlank() && (
            ch == a ||
                ch.startsWith("$a ") ||
                ch.contains("$a -") ||
                ch.contains("$a official") ||
                ch.contains("official $a")
            )
